...
(() => {
  ...
  css.textContent = \`
    ...
    .device-diagnostics { display: none !important; box-shadow: 0 16px 50px rgba(0,0,0,.42); }
  ...
  function patchRetryButton() {
    ...
    retry.addEventListener('click', async () => {
      setText('selected-device-label', 'Retry Detect running.');
      openFallbackGrid();
      setTimeout(() => {
  ...
  function patchManualButton() {
    ...
    manual.addEventListener('click', () => {
      openFallbackGrid();
      setText('selected-device-label', 'Choose QuickTime Movie Recording or another visible source.');
      setTimeout(() => {
        if (typeof window.scanSources === 'function') window.scanSources('device', false, true);
      }, 100);
    }, true);
